...
export const PropertySearch = () => {
  ...
  const search = async () => {
    ...
    if ("true" === hasParking) filters.hasParking = true;
    if ("true" === petFriendly) filters.petFriendly = true;
  ...
  const handlePageClick = async (pageNumber) => {
    ...
    await router.push(`${router.query.slug.join("/")}?page=${pageNumber}&petFriendly=${"true" === petFriendly}&hasParking=${"true" === hasParking}&minPrice=${minPrice}&maxPrice=${maxPrice}`, null, {
      shallow: true
    });
    search();
  };
